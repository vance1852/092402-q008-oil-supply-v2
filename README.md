...
- 不可抗力案件冻结证据、影响窗口和容量上限版本，按合同排序确定性削减尚未发运的配额并退回库存预留，在途数量保持不变；撤销、延长、提前结束和申诉裁决都形成后继版本，逐版给出数量守恒对账；
- 每家托运方在申诉截止前只能申诉一次，裁决只恢复本家份额，其他已生效份额冻结不变；
...
- `src/oil_supply/`：报价、设施、线路、库存、提名、不可抗力案件、供应情景、HTTP API 与离线验收；
...
健康检查为 `GET /health`。除健康检查外，请求通过 `X-Actor-Id` 携带操作者编号。可用接口覆盖报价、设施、线路、停运事件、库存批次、提名、能力分配、发运、不可抗力案件与申诉、供应情景和审计链。不可抗力相关接口包括：

- `POST /force-majeure`：风险人员宣布案件，冻结证据、影响窗口和容量上限并执行首版削减；
- `POST /force-majeure/{case_id}/extend|cancel|end`：延长、撤销或提前结束，均产生后继版本并要求 `idempotency_key`；
- `POST /appeals` 与 `POST /appeals/{appeal_id}/rule`：托运方在截止前申诉一次，风险人员裁决（可随裁决上调容量上限）；
- `GET /force-majeure/{case_id}`：案件全部版本、每版数量守恒对账与申诉记录；
- `GET /nominations/{nomination_id}/force-majeure`：托运方从一条提名直达案件、合同排序、每版削减/恢复数量、在途不变量和申诉结果。

服务重启后，SQLite 中的业务状态和历史版本会继续保留。
